...
export default {
  setup() {
    ...
    onMounted(async () => {
      try {
        // NOTE if you want to handle a user LEAVING a room, you're going to want a life cycle hook for onRouteLeave or onUnMounted,
        AppState.donations = []
        ...
        // NOTE join the room on page load
        socketService.joinRoom(`campaign-${route.params.id}`)
      } catch (error) {
        logger.error(error)
    ...
    onBeforeRouteLeave(() => {
      try {
        socketService.leaveRoom(`campaign-${route.params.id}`)
      } catch (error) {
        logger.error(error)
        Pop.toast(error.message, 'error')
      }
